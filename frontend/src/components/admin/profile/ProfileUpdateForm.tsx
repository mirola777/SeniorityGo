...
function ProfileUpdateForm({ onUpdateProfile, profile }: ProfileUpdateFormProps) {
    ...
    const [profileDict, setProfileDict] = useState({
        ...
        seniorities: [{ seniority: 0, pokemon: 0, requirements: [{ requirement: 0 }] }]
    });
    ...
    const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
        ...
        updateProfile(profile.getId(), profileDict).then((profileResponse) => {

            setSuccess(t('profile_updated'));
            onUpdateProfile(profileResponse);
            setErrors([]);
        }).catch((error) => {
        ...
        getUserSession().then((user) => {
            ...
            getOrganizationSeniorities().then((seniorities) => {
                ...
                setProfileDict({
                    ...
                    name: profile.getName(),
                    description: profile.getDescription(),
                    organization: user.getUser().getOrganization(),
                    seniorities: profile.getProfilesSeniorities().map((profileseniority) => {
                        return {
                            seniority: profileseniority?.getSeniority()?.getId() ?? 0,
                            pokemon: profileseniority?.getPokemon()?.getId() ?? 0,
                            requirements: profileseniority.getRequirements().map((requirement) => {
                                return {
